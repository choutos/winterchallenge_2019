...
def computer(intcode: list, noun: int = None, verb: int = None, alarm=False) -> list:
    pos = 0
    if noun:
        intcode[1] = noun
    if verb:
...
for noun in range(0,100):
    for verb in range(0,100):
        data = getData()
        if computer(data, noun, verb, alarm=True) == 19690720:
            print("Part2: {}".format(100 * noun + verb))
            break

    
#print(compute(intcode,noun=2,verb=12,alarm=True))
